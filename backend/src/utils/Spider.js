...
class Spider {
  ...
  enqueue(domain, fn) {
    return new Promise((resolve, reject) => {
      ...
      if (!st) { st = { queue: [], running: false, nextAt: 0 }; this.domainState.set(domain, st); }
  ...
  async start(seedUrl, seedHtml, seedUserAgent = '') {
    try {
      ...
      for (const url of frontier) { if (visited.has(url)) continue; allToFetch.add(url); }
            ...
            try {
              ...
              const res = await this.enqueue(domain, async () => {
                ...
                return axios.get(url, {
                  ...
                  maxRedirects: 3,
                  validateStatus: () => true,
                  headers: { 'User-Agent': defaultUA, 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8' },
                });
              });
