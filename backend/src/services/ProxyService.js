...
import { startSpider } from '../utils/Spider.js';
  ...
  _setupProxyHandlers() {
    ...
    this.proxy.on('proxyRes', (proxyRes, req, res) => {
      ...
      proxyRes.on('end', () => {
        ...
        try { console.log(`[PROXY] ${record.method} ${record.status} ${record.url} (path=${record.path || '/'})`); } catch {}
        ...
        try { this.queueCveLookup(proxyRes.headers); } catch {}
        
        // Track host attribution for tech@version pairs
        try { this.registerTechVersionsForHost(proxyRes.headers, record.host); } catch {}
